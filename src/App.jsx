...
function loadScript(src, check) {
  return new Promise(r => { if (check()) { r(); return; } const s = document.createElement("script"); s.src = src; s.onload = r; document.head.appendChild(s); });
}
...
async function initGis() { await loadScript("https://accounts.google.com/gsi/client", () => !!window.google?.accounts); }
function saveToken(t) { localStorage.setItem(TOKEN_KEY, t.access_token); localStorage.setItem(TOKEN_EXP, String(Date.now() + (t.expires_in - 60) * 1000)); window.gapi.client.setToken(t); }
function loadCachedToken() { const t = localStorage.getItem(TOKEN_KEY), e = Number(localStorage.getItem(TOKEN_EXP)); if (t && e && Date.now() < e) { window.gapi.client.setToken({ access_token: t }); return true; } return false; }
function clearToken() { localStorage.removeItem(TOKEN_KEY); localStorage.removeItem(TOKEN_EXP); window.gapi.client.setToken(null); }
...
async function readJsonFile(id) { const r = await window.gapi.client.drive.files.get({ fileId: id, alt: "media" }); return typeof r.result === "string" ? JSON.parse(r.result) : r.result; }
async function createJsonFile(name, data, parents = ["appDataFolder"]) {
  const b = "ck_b", body = [`--${b}`, "Content-Type: application/json; charset=UTF-8", "", JSON.stringify({ name, parents }), `--${b}`, "Content-Type: application/json", "", JSON.stringify(data), `--${b}--`].join("\r\n");
...
async function updateJsonFile(id, data) { await window.gapi.client.request({ path: `https://www.googleapis.com/upload/drive/v3/files/${id}`, method: "PATCH", params: { uploadType: "media" }, headers: { "Content-Type": "application/json" }, body: JSON.stringify(data) }); }
async function getOrCreateDriveFolder() { const e = await findFile(FOLDER_NAME, "drive"); if (e) return e.id; const r = await window.gapi.client.drive.files.create({ resource: { name: FOLDER_NAME, mimeType: "application/vnd.google-apps.folder" }, fields: "id" }); return r.result.id; }
...
const C = { bg:"#080c14", surface:"#0f1521", surface2:"#161e2e", surface3:"#1c2640", border:"rgba(255,255,255,0.06)", border2:"rgba(255,255,255,0.10)", accent:"#818cf8", text1:"#f1f5f9", text2:"#8892a4", text3:"#4a5568", danger:"#f87171", success:"#34d399", warning:"#fbbf24" };
...
const EVENT_TYPES = { exam:{label:"\uC2DC\uD5D8",color:"#f87171"}, study:{label:"\uD559\uC2B5",color:"#818cf8"}, cert:{label:"\uC790\uACA9\uC99D",color:"#34d399"}, other:{label:"\uAE30\uD0C0",color:"#8892a4"} };
...
function FileViewer({ file, onClose }) {
    ...
    (async () => {
      ...
      try {
        ...
        if (isText(file.name)) {
          ...
          setBlobUrl(url);
          setState(isImage(file.name) ? "image" : isPdf(file.name) ? "pdf" : "other");
        }
      } catch (e) {
        setState("error");
      }
  ...
  const FileIcon = isImage(file.name) ? Image : isPdf(file.name) ? FileText : File;
  ...
  return (
    <div onClick={e=>e.target===e.currentTarget&&onClose()} style={{ position:"fixed", inset:0, zIndex:60, background:"rgba(0,0,0,0.85)", backdropFilter:"blur(12px)", display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:12, padding:"12px 20px", background:C.surface, borderBottom:`1px solid ${C.border2}`, flexShrink:0 }}>
        <div style={{ borderRadius:8, padding:6, background:C.accent+"18" }}><FileIcon size={14} color={C.accent}/></div>
        ...
        {blobUrl && (
          <a href={blobUrl} download={file.name} style={{ display:"flex", alignItems:"center", gap:6, fontSize:12, color:C.text2, padding:"6px 12px", borderRadius:8, border:`1px solid ${C.border2}`, textDecoration:"none" }}>
            <Download size={12}/>{"\uB2E4\uC6B4\uB85C\uB4DC"}
          </a>
        )}
        {file.webViewLink && (
          <a href={file.webViewLink} target="_blank" rel="noreferrer" style={{ display:"flex", alignItems:"center", gap:6, fontSize:12, color:C.text2, padding:"6px 12px", borderRadius:8, border:`1px solid ${C.border2}`, textDecoration:"none" }}>
            <ExternalLink size={12}/>Drive{"\uC5D0\uC11C \uBCF4\uAE30"}
          </a>
        )}
      ...
      <div style={{ flex:1, overflow:"auto", display:"flex", alignItems:"center", justifyContent:"center", padding:24 }}>
        ...
        {state==="no-drive" && <div style={{ textAlign:"center" }}><p style={{ fontSize:14, color:C.text2 }}>{"\uB85C\uCEEC \uD30C\uC77C\uC785\uB2C8\uB2E4. \uBDF0\uC5B4\uB97C \uC9C0\uC6D0\uD558\uC9C0 \uC54A\uC2B5\uB2C8\uB2E4."}</p></div>}
        ...
        {state==="text" && (
          <div style={{ width:"100%", maxWidth:800, background:C.surface, borderRadius:12, padding:24, border:`1px solid ${C.border2}` }}>
            <pre style={{ fontSize:12, color:C.text2, lineHeight:1.7, whiteSpace:"pre-wrap", wordBreak:"break-word", overflowY:"auto", maxHeight:"70vh" }}>{textContent}</pre>
          </div>
        )}
        {state==="other" && (
          <div style={{ textAlign:"center" }}>
            <div style={{ borderRadius:20, padding:24, background:C.surface2, display:"inline-flex", marginBottom:16 }}><File size={40} color={C.text3}/></div>
            <p style={{ fontSize:13, color:C.text2, marginBottom:16 }}>{"\uC774 \uD615\uC2DD\uC740 \uBDF0\uC5B4\uC5D0\uC11C \uC9C1\uC811 \uBCF4\uAE30\uAC00 \uBD88\uAC00\uB2A5\uD569\uB2C8\uB2E4."}</p>
            <div style={{ display:"flex", gap:8, justifyContent:"center" }}>
              {blobUrl&&<a href={blobUrl} download={file.name} style={{ display:"flex", alignItems:"center", gap:6, fontSize:13, color:"white", padding:"8px 16px", borderRadius:10, background:C.accent, textDecoration:"none" }}><Download size={13}/>{"\uB2E4\uC6B4\uB85C\uB4DC"}</a>}
              {file.webViewLink&&<a href={file.webViewLink} target="_blank" rel="noreferrer" style={{ display:"flex", alignItems:"center", gap:6, fontSize:13, color:C.text2, padding:"8px 16px", borderRadius:10, border:`1px solid ${C.border2}`, textDecoration:"none" }}><ExternalLink size={13}/>Drive{"\uC5D0\uC11C \uC5F4\uAE30"}</a>}
            </div>
          </div>
        )}
      </div>
...
function EmptyState({ icon:Icon, title, sub, action }) {
  return <div style={{ display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"64px 24px",border:`1.5px dashed ${C.border2}`,borderRadius:20,textAlign:"center" }}><div style={{ background:C.surface2,borderRadius:16,padding:16,marginBottom:16 }}><Icon size={28} color={C.text3}/></div><p style={{ fontSize:14,color:C.text2,fontWeight:500,marginBottom:6 }}>{title}</p>{sub&&<p style={{ fontSize:12,color:C.text3,marginBottom:16 }}>{sub}</p>}{action}</div>;
}
function PageHeader({ title, sub, action }) {
  return <div style={{ display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:32 }}><div><h2 style={{ fontSize:20,fontWeight:700,color:C.text1,fontFamily:"Georgia,serif" }}>{title}</h2>{sub&&<p style={{ fontSize:13,color:C.text2,marginTop:4 }}>{sub}</p>}</div>{action}</div>;
}
...
function Dashboard({ library, certCategories, events, setPage, userInfo }) {
  const totalFiles = library.reduce((a,s)=>{
    function countFiles(node) { return (node.files||[]).length + (node.folders||[]).reduce((b,f)=>b+countFiles(f),0); }
    return a + countFiles(s);
  }, 0);
    ...
    <div>
      ...
      <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:16 }}>
        ...
        <div style={{ ...S.card,padding:20 }}>
          ...
          {upcomingEvents.length?(<div style={{ display:"flex",flexDirection:"column",gap:8 }}>{upcomingEvents.map(e=>(<div key={e.id} style={{ display:"flex",alignItems:"center",gap:8 }}><div style={{ width:6,height:6,borderRadius:"50%",background:EVENT_TYPES[e.type]?.color,flexShrink:0 }}/><span style={{ fontSize:12,flex:1,color:C.text2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{e.title}</span><span style={{ fontSize:11,color:C.text3,flexShrink:0 }}>{formatDate(e.date)}</span></div>))}</div>):<p style={{ fontSize:13,color:C.text2 }}>{"\uC608\uC815\uB41C \uC77C\uC815\uC774 \uC5C6\uC2B5\uB2C8\uB2E4."}</p>}
        </div>
      </div>
    </div>
  );
...
function FileRow({ file, color, onDelete, onRename, onView, deleting, depth=0 }) {
  ...
  async function saveRename() {
    if (!newName.trim() || newName===file.name) { setEditing(false); return; }
    setRenaming(true);
    await onRename(file, newName.trim());
    setRenaming(false); setEditing(false);
  }
    ...
    <div style={{ display:"flex",alignItems:"center",gap:10,padding:"8px 16px",paddingLeft:16+depth*20,borderBottom:`1px solid ${C.border}` }}>
      ...
      {editing ? (
        <input autoFocus value={newName} onChange={e=>setNewName(e.target.value)}
          onKeyDown={e=>{if(e.key==="Enter")saveRename();if(e.key==="Escape"){setEditing(false);setNewName(file.name);}}}
          style={{ flex:1,background:"transparent",border:"none",borderBottom:`1px solid ${C.accent}`,color:C.text1,fontSize:12,outline:"none",fontFamily:"inherit" }}/>
      ) : (
      ...
      {editing ? (
        ...
        <>
          <button onClick={()=>onView(file)} style={{ background:"transparent",border:"none",cursor:"pointer",padding:3,color:C.accent,display:"flex" }}><Eye size={11}/></button>
          <button onClick={()=>{setEditing(true);setNewName(file.name);}} style={{ background:"transparent",border:"none",cursor:"pointer",padding:3,color:C.text2,display:"flex" }}><Pencil size={11}/></button>
...
function FolderTree({ folder, sectionId, sectionColor, depth=0, onDeleteFile, onRenameFile, onViewFile, onDeleteFolder, onRenameFolder, onAddSubfolder, onUpload, deletingFile, uploading, uploadTarget }) {
  ...
  function saveRename() {
    ...
    onRenameFolder(sectionId, folder.id, editName.trim(), depth);
    setEditing(false);
  }
  function addSub() {
    if (!subName.trim()) return;
    onAddSubfolder(sectionId, folder.id, subName.trim(), depth);
      ...
      <div style={{ display:"flex",alignItems:"center",gap:8,padding:`10px 16px`,paddingLeft:indentPx,background:depth%2===0?C.surface2:C.surface3 }}>
        ...
        {editing?(
          <input autoFocus value={editName} onChange={e=>setEditName(e.target.value)}
            ...
            style={{ flex:1,background:"transparent",border:"none",borderBottom:`1px solid ${sectionColor}`,color:C.text1,fontSize:12,fontWeight:500,outline:"none",fontFamily:"inherit" }}/>
        ):(
        ...
        {editing?(
          <button onClick={saveRename} style={{ background:"transparent",border:"none",cursor:"pointer",padding:3,color:sectionColor,display:"flex" }}><Check size={11}/></button>
        ):(
          <button onClick={()=>{setEditing(true);setEditName(folder.name);}} style={{ background:"transparent",border:"none",cursor:"pointer",padding:3,color:C.text2,display:"flex" }}><Pencil size={11}/></button>
        )}
        <button onClick={()=>setShowAddSub(true)} title="\uD558\uC704 \uD3F4\uB354 \uCD94\uAC00" style={{ background:"transparent",border:"none",cursor:"pointer",padding:3,color:C.text2,display:"flex" }}><FolderOpen size={11}/></button>
        <button onClick={()=>onUpload(sectionId, folder.id)} disabled={isUploading} style={{ background:"transparent",border:"none",cursor:"pointer",padding:3,color:sectionColor,display:"flex" }}>
          {isUploading?<Loader2 size={11} style={{ animation:"spin 1s linear infinite" }}/>:<Upload size={11}/>}
        </button>
        <button onClick={()=>onDeleteFolder(sectionId, folder.id, depth)} style={{ background:"transparent",border:"none",cursor:"pointer",padding:3,color:C.danger,display:"flex" }}><Trash2 size={11}/></button>
      </div>

      {/* Add subfolder input */}
      {showAddSub && (
        <div style={{ display:"flex",gap:8,padding:"8px 16px",paddingLeft:indentPx+20,background:C.surface2,borderTop:`1px solid ${C.border}` }}>
          <input autoFocus value={subName} onChange={e=>setSubName(e.target.value)}
            ...
            placeholder={"\uD558\uC704 \uD3F4\uB354 \uC774\uB984 \uC785\uB825 \u2192 Enter"}
            style={{ ...S.input,flex:1,fontSize:11,padding:"5px 8px" }}/>
          <button onClick={addSub} style={{ background:C.accent,border:"none",borderRadius:8,color:"white",fontSize:11,padding:"5px 10px",cursor:"pointer",fontFamily:"inherit" }}>{"\uCD94\uAC00"}</button>
          <button onClick={()=>setShowAddSub(false)} style={{ background:"transparent",border:"none",cursor:"pointer",color:C.text3 }}><X size={12}/></button>
        </div>
      )}
      ...
      {!collapsed && (
        <>
          {(folder.files||[]).map(file=>(
            <FileRow key={file.id} file={file} color={sectionColor} depth={depth+1}
              onDelete={f=>onDeleteFile(sectionId,f,folder.id)}
              onRename={(f,n)=>onRenameFile(sectionId,f,n,folder.id)}
              onView={onViewFile} deleting={deletingFile}/>
          ))}
          {(folder.folders||[]).map(sub=>(
            <FolderTree key={sub.id} folder={sub} sectionId={sectionId} sectionColor={sectionColor} depth={depth+1}
              onDeleteFile={onDeleteFile} onRenameFile={onRenameFile} onViewFile={onViewFile}
              onDeleteFolder={onDeleteFolder} onRenameFolder={onRenameFolder}
              onAddSubfolder={onAddSubfolder} onUpload={onUpload}
              deletingFile={deletingFile} uploading={uploading} uploadTarget={uploadTarget}/>
          ))}
          {(folder.files||[]).length===0&&(folder.folders||[]).length===0&&(
            <div style={{ padding:`8px 16px`,paddingLeft:indentPx+20,fontSize:11,color:C.text3 }}>{"\uBE44\uC5B4 \uC788\uC2B5\uB2C8\uB2E4."}</div>
          )}
        </>
      )}
    </div>
...
function Library({ library, onChange, driveFolderId }) {
  ...
  // ── helpers to update nested folder by id ──
  function updateFolderInTree(folders, targetId, updater) {
    return folders.map(f => {
      if (f.id === targetId) return updater(f);
      if (f.folders?.length) return { ...f, folders: updateFolderInTree(f.folders, targetId, updater) };
      return f;
    });
  }
  function removeFolderFromTree(folders, targetId) {
    return folders.filter(f=>f.id!==targetId).map(f=>({ ...f, folders: f.folders ? removeFolderFromTree(f.folders,targetId) : [] }));
  }
  function findFolderInTree(folders, targetId) {
    for (const f of folders) { if (f.id===targetId) return f; if (f.folders?.length) { const found=findFolderInTree(f.folders,targetId); if(found) return found; } } return null;
  }
  ...
  function addSection() {
    ...
    setNewSection({ subject:"",color:SEC_COLORS[0] }); setShowAddSection(false);
  }
  ...
  // ── Root folder ops ──
  function addRootFolder(sectionId) {
    if (!newFolderName.trim()) return;
    onChange(library.map(s=>s.id!==sectionId?s:{...s,folders:[...(s.folders||[]),{ id:uid(),name:newFolderName.trim(),files:[],folders:[] }]}));
    setNewFolderName(""); setAddFolderTarget(null);
  }

  // ── Recursive folder ops ──
  function handleAddSubfolder(sectionId, parentFolderId, name) {
    onChange(library.map(s=>{
      if (s.id!==sectionId) return s;
      const newFolders = updateFolderInTree(s.folders||[], parentFolderId, f=>({ ...f, folders:[...(f.folders||[]),{ id:uid(),name,files:[],folders:[] }] }));
      return { ...s, folders:newFolders };
    }));
  }
  function handleDeleteFolder(sectionId, folderId) {
    onChange(library.map(s=>{
      if (s.id!==sectionId) return s;
      return { ...s, folders:removeFolderFromTree(s.folders||[],folderId) };
    }));
  }
  function handleRenameFolder(sectionId, folderId, name) {
    onChange(library.map(s=>{
      if (s.id!==sectionId) return s;
      return { ...s, folders:updateFolderInTree(s.folders||[],folderId,f=>({...f,name})) };
    }));
  }

  // ── Upload ──
  function openUpload(sectionId, folderId=null) { setUploadTarget({ sectionId, folderId }); setTimeout(()=>fileInputRef.current?.click(),50); }
  ...
  // ── File ops ──
  async function handleDeleteFile(sectionId, file, folderId=null) {
    setDeletingFile(file.id);
    try {
      if (file.driveId) await deleteDriveFile(file.driveId);
      onChange(library.map(s=>{
        if (s.id!==sectionId) return s;
        if (!folderId) return { ...s, files:(s.files||[]).filter(f=>f.id!==file.id) };
        return { ...s, folders:updateFolderInTree(s.folders||[],folderId,f=>({...f,files:(f.files||[]).filter(fi=>fi.id!==file.id)})) };
      }));
    } catch(err) { console.error(err); } finally { setDeletingFile(null); }
  }
  async function handleRenameFile(sectionId, file, newName, folderId=null) {
    if (file.driveId) { try { await renameDriveFile(file.driveId, newName); } catch(e) { console.error(e); } }
    onChange(library.map(s=>{
      if (s.id!==sectionId) return s;
      const renamer = files => files.map(f=>f.id===file.id?{...f,name:newName}:f);
      if (!folderId) return { ...s, files:renamer(s.files||[]) };
      return { ...s, folders:updateFolderInTree(s.folders||[],folderId,f=>({...f,files:renamer(f.files||[])})) };
    }));
  }
  ...
  return (
    <div>
      ...
      <PageHeader title={"\uAC15\uC758 \uC790\uB8CC\uC2E4"} sub={"\uC139\uC158 \u2192 \uD3F4\uB354(\uC911\uCCA9 \uAC00\uB2A5) \u2192 \uD30C\uC77C"}
            ...
            <div key={section.id} style={{ ...S.card,overflow:"hidden" }}>
              ...
              <div style={{ display:"flex",alignItems:"center",gap:10,padding:"14px 20px",borderLeft:`3px solid ${section.color}`,background:C.surface,borderBottom:`1px solid ${C.border}` }}>
                ...
                {editSectionId===section.id?(
                  ...
                  <button onClick={()=>{setEditSectionId(section.id);setEditSectionName(section.subject);}} style={{ background:"transparent",border:"none",cursor:"pointer",padding:4,color:C.text2,display:"flex" }}><Pencil size={12}/></button>
                )}
      ...
      {showAddSection&&(
        <Modal title={"\uC0C8 \uC139\uC158 \uCD94\uAC00"} onClose={()=>setShowAddSection(false)}>
          <div style={{ ...S.col,gap:16 }}>
            <Input label={"\uACFC\uBAA9\uBA85"} value={newSection.subject} onChange={e=>setNewSection(p=>({...p,subject:e.target.value}))} onKeyDown={e=>e.key==="Enter"&&addSection()} placeholder="NCS, TOEIC ..."/>
            <Field label={"\uC0C9\uC0C1"}><div style={{ display:"flex",gap:8,flexWrap:"wrap" }}>{SEC_COLORS.map(c=>(<button key={c} onClick={()=>setNewSection(p=>({...p,color:c}))} style={{ width:28,height:28,borderRadius:"50%",background:c,border:newSection.color===c?"3px solid white":"3px solid transparent",cursor:"pointer",outline:"none",opacity:newSection.color===c?1:0.5 }}/>))}</div></Field>
            <div style={{ display:"flex",gap:8,justifyContent:"flex-end" }}><Btn variant="ghost" onClick={()=>setShowAddSection(false)}>{"\uCDE8\uC18C"}</Btn><Btn onClick={addSection}>{"\uCD94\uAC00"}</Btn></div>
          </div>
        </Modal>
      )}
      {addFolderTarget&&(
        <Modal title={"\uD3F4\uB354 \uCD94\uAC00"} onClose={()=>setAddFolderTarget(null)}>
          <div style={{ ...S.col,gap:16 }}>
            <Input label={"\uD3F4\uB354 \uC774\uB984"} value={newFolderName} onChange={e=>setNewFolderName(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addRootFolder(addFolderTarget)} placeholder="1\uC8FC\uCC28, \uC2E4\uC804\uBB38\uC81C ..."/>
            <div style={{ display:"flex",gap:8,justifyContent:"flex-end" }}><Btn variant="ghost" onClick={()=>setAddFolderTarget(null)}>{"\uCDE8\uC18C"}</Btn><Btn onClick={()=>addRootFolder(addFolderTarget)}>{"\uCD94\uAC00"}</Btn></div>
          </div>
        </Modal>
      )}
    </div>
...
function Certificates({ certCategories, onChange, driveFolderId }) {
  ...
  async function handleFileSelect(e) {
    const files=Array.from(e.target.files); if(!files.length||!driveFolderId) return;
  ...
  async function handleDeleteFile(catId, certId, file) {
    setDeletingFile(file.id);
    try { if(file.driveId) await deleteDriveFile(file.driveId); onChange(certCategories.map(cat=>cat.id!==catId?cat:{...cat,certs:(cat.certs||[]).map(c=>c.id!==certId?c:{...c,files:(c.files||[]).filter(f=>f.id!==file.id)})})); }
    catch(err) { console.error(err); } finally { setDeletingFile(null); }
  }
  async function handleRenameFile(catId, certId, file, newName) {
    if (file.driveId) { try { await renameDriveFile(file.driveId, newName); } catch(e) { console.error(e); } }
    onChange(certCategories.map(cat=>cat.id!==catId?cat:{...cat,certs:(cat.certs||[]).map(c=>c.id!==certId?c:{...c,files:(c.files||[]).map(f=>f.id===file.id?{...f,name:newName}:f)})}));
  }
  ...
  return (
    <div>
      <input ref={fileInputRef} type="file" multiple style={{ display:"none" }} onChange={handleFileSelect}/>
      {viewingFile&&<FileViewer file={viewingFile} onClose={()=>setViewingFile(null)}/>}
      <PageHeader title={"\uC790\uACA9\uC99D \uBCF4\uAD00\uD568"} sub={"\uCE74\uD14C\uACE0\uB9AC \u2192 \uC790\uACA9\uC99D \u2192 \uAD00\uB828 \uD30C\uC77C"}
                                ...
                                {(cert.files||[]).map(file=>{
                                  return (
                                    <div key={file.id} style={{ display:"flex",alignItems:"center",gap:8,padding:"6px 14px",borderBottom:"1px solid rgba(255,255,255,0.05)" }}>
                                      ...
                                      <button onClick={()=>setViewingFile(file)} style={{ background:"transparent",border:"none",cursor:"pointer",display:"flex",color:"rgba(255,255,255,0.5)",padding:2 }}><Eye size={10}/></button>
                                      <button onClick={()=>handleDeleteFile(cat.id,cert.id,file)} disabled={deletingFile===file.id} style={{ background:"transparent",border:"none",cursor:"pointer",display:"flex",color:"rgba(255,255,255,0.4)",padding:2 }}>{deletingFile===file.id?<Loader2 size={10} style={{ animation:"spin 1s linear infinite" }}/>:<Trash2 size={10}/>}</button>
                                    </div>
                                  );
                                })}
                              </div>
...
function Scheduler({ events, onChange }) {
  ...
  function addEvent() { if(!form.title.trim()||!form.date) return; onChange([...events,{...form,id:uid(),isDday:form.isDday}]); setForm({ title:"",date:today(),type:"exam",note:"",isDday:true }); setShowAdd(false); }
  ...
  return (
    <div>
      <div style={{ display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:32 }}>
        <div><h2 style={{ fontSize:20,fontWeight:700,color:C.text1,fontFamily:"Georgia,serif" }}>{"\uD559\uC2B5 \uC2A4\uCF00\uC904\uB7EC"}</h2><p style={{ fontSize:13,color:C.text2,marginTop:4 }}>{"\uC2DC\uD5D8 \uC77C\uC815\uACFC D-Day\uB97C \uAD00\uB9AC\uD569\uB2C8\uB2E4"}</p></div>
      ...
      {view==="month"&&(<div style={{ ...S.card,overflow:"hidden",marginBottom:24 }}><div style={{ display:"grid",gridTemplateColumns:"repeat(7,1fr)" }}>{WEEKDAYS.map((d,i)=>(<div key={d} style={{ padding:"10px 0",textAlign:"center",fontSize:11,fontWeight:600,color:i===0?C.danger:i===6?C.accent:C.text3,background:C.surface,borderBottom:`1px solid ${C.border}` }}>{d}</div>))}{monthGrid.map((day,i)=>{ const evs=eventsOn(day),ds=day?.toISOString().split("T")[0],isToday=ds===todayStr,isWeekend=day&&(day.getDay()===0||day.getDay()===6); return (<div key={i} onClick={()=>day&&openAdd(ds)} style={{ minHeight:80,padding:6,cursor:day?"pointer":"default",background:day?"transparent":C.surface2,borderRight:(i+1)%7!==0?`1px solid ${C.border}`:"none",borderBottom:i<monthGrid.length-7?`1px solid ${C.border}`:"none" }}>{day&&(<><div style={{ display:"flex",justifyContent:"flex-end",marginBottom:2 }}><span style={{ fontSize:11,width:22,height:22,display:"flex",alignItems:"center",justifyContent:"center",borderRadius:"50%",fontWeight:500,background:isToday?C.accent:"transparent",color:isToday?"white":isWeekend?(day.getDay()===0?C.danger:C.accent):C.text2 }}>{day.getDate()}</span></div><div style={{ display:"flex",flexDirection:"column",gap:2 }}>{evs.slice(0,2).map(e=>(<div key={e.id} onClick={ev=>{ev.stopPropagation();deleteEvent(e.id);}} style={{ fontSize:10,borderRadius:4,padding:"1px 4px",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",background:EVENT_TYPES[e.type]?.color+"22",color:EVENT_TYPES[e.type]?.color,cursor:"pointer" }}>{e.isDday&&"\u2605 "}{e.title}</div>))}{evs.length>2&&<div style={{ fontSize:10,color:C.text3,paddingLeft:4 }}>+{evs.length-2}</div>}</div></>)}</div>); })}</div></div>)}
      {view==="week"&&(<div style={{ ...S.card,overflow:"hidden",marginBottom:24 }}><div style={{ display:"grid",gridTemplateColumns:"repeat(7,1fr)" }}>{weekDates.map((day,i)=>{ const ds=day.toISOString().split("T")[0],isToday=ds===todayStr,evs=eventsOn(day); return (<div key={i} style={{ borderRight:i<6?`1px solid ${C.border}`:"none" }}><div onClick={()=>openAdd(ds)} style={{ padding:"10px 0",textAlign:"center",cursor:"pointer",background:C.surface,borderBottom:`1px solid ${C.border}` }}><div style={{ fontSize:11,color:i===0?C.danger:i===6?C.accent:C.text3,marginBottom:4 }}>{WEEKDAYS[i]}</div><div style={{ fontSize:16,fontWeight:700,width:32,height:32,display:"flex",alignItems:"center",justifyContent:"center",borderRadius:"50%",margin:"0 auto",background:isToday?C.accent:"transparent",color:isToday?"white":C.text1 }}>{day.getDate()}</div></div><div style={{ padding:4,minHeight:120,display:"flex",flexDirection:"column",gap:4 }}>{evs.map(e=>(<div key={e.id} onClick={()=>deleteEvent(e.id)} style={{ fontSize:11,borderRadius:6,padding:"4px 6px",background:EVENT_TYPES[e.type]?.color+"22",color:EVENT_TYPES[e.type]?.color,cursor:"pointer",fontWeight:500 }}>{e.isDday&&"\u2605 "}{e.title}</div>))}</div></div>); })}</div></div>)}
      <div>
        ...
        {events.length===0?<p style={{ fontSize:13,color:C.text2 }}>{"\uC77C\uC815\uC774 \uC5C6\uC2B5\uB2C8\uB2E4."}</p>:(
          <div style={{ display:"flex",flexDirection:"column",gap:8 }}>
            {[...events].sort((a,b)=>new Date(a.date)-new Date(b.date)).map(e=>{ const diff=diffDays(e.date); return (<div key={e.id} style={{ display:"flex",alignItems:"center",gap:10,padding:"12px 16px",...S.card }}><div style={{ width:8,height:8,borderRadius:"50%",background:EVENT_TYPES[e.type]?.color,flexShrink:0 }}/><div style={{ flex:1,minWidth:0 }}><div style={{ fontSize:13,fontWeight:500,color:C.text1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{e.isDday&&<span style={{ color:C.accent,marginRight:4 }}>★</span>}{e.title}</div>{e.note&&<div style={{ fontSize:11,color:C.text3,marginTop:2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{e.note}</div>}</div><span style={{ fontSize:11,color:C.text3,flexShrink:0 }}>{formatDate(e.date)}</span>{e.isDday&&<span style={{ fontSize:11,fontWeight:700,flexShrink:0,padding:"2px 8px",borderRadius:99,background:diff<0?"rgba(148,163,184,0.1)":diff===0?C.danger+"20":C.accent+"18",color:diff<0?C.text3:diff===0?C.danger:C.accent }}>{dDayLabel(diff)}</span>}<button onClick={()=>deleteEvent(e.id)} style={{ background:"transparent",border:"none",cursor:"pointer",padding:4,color:C.danger,display:"flex",flexShrink:0 }}><Trash2 size={12}/></button></div>); })}
          </div>
        )}
      </div>
      {showAdd&&(<Modal title={"\uC77C\uC815 \uCD94\uAC00"} onClose={()=>setShowAdd(false)}><div style={{ ...S.col,gap:12 }}><Input label={"\uC81C\uBAA9 *"} value={form.title} onChange={fld("title")} onKeyDown={e=>e.key==="Enter"&&addEvent()} placeholder="TOEIC \uC2DC\uD5D8 ..."/><div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10 }}><Input label={"\uB0A0\uC9DC *"} type="date" value={form.date} onChange={fld("date")}/><SelectInput label={"\uC720\uD615"} value={form.type} onChange={fld("type")}>{Object.entries(EVENT_TYPES).map(([k,v])=><option key={k} value={k}>{v.label}</option>)}</SelectInput></div><Textarea label={"\uBA54\uBAA8"} value={form.note} onChange={fld("note")} placeholder="\uC7A5\uC18C, \uC900\uBE44\uBB3C ..."/><label style={{ display:"flex",alignItems:"center",gap:10,cursor:"pointer" }}><div style={{ position:"relative",width:40,height:22,flexShrink:0 }} onClick={()=>setForm(p=>({...p,isDday:!p.isDday}))}><div style={{ position:"absolute",inset:0,borderRadius:99,background:form.isDday?C.accent:C.surface3,transition:"background 0.2s" }}/><div style={{ position:"absolute",top:2,left:form.isDday?20:2,width:18,height:18,borderRadius:"50%",background:"white",transition:"left 0.2s" }}/></div><span style={{ fontSize:13,color:C.text2 }}>D-Day \uCE74\uC6B4\uD2B8\uB2E4\uC6B4 \uD45C\uC2DC</span></label><div style={{ display:"flex",gap:8,justifyContent:"flex-end",marginTop:4 }}><Btn variant="ghost" onClick={()=>setShowAdd(false)}>{"\uCDE8\uC18C"}</Btn><Btn onClick={addEvent}>{"\uCD94\uAC00"}</Btn></div></div></Modal>)}
    </div>
  );
}
...
export default function App() {
  ...
  async function bootstrapApp() {
    ...
    if (existing) {
      ...
      if (loaded.certs&&!loaded.certCategories) {
        setData({ library:loaded.library||[], certCategories:[{ id:uid(),name:"\uAE30\uD0C8",color:SEC_COLORS[0],certs:loaded.certs.map(c=>({...c,files:c.files||[]})) }], events:loaded.events||[] });
      } else {
  ...
  function scheduleSave(d) {
    ...
    saveTimerRef.current=setTimeout(async()=>{ try { await updateJsonFile(dataFileIdRef.current,d); setSyncStatus("synced"); } catch(e) { console.error(e); setSyncStatus("error"); } },1500);
  }
