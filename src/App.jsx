/**
 * CareerManager.jsx — Google Drive 연동 버전
 *
 * 데이터 저장 방식:
 *   - Google Drive에 "career_data.json" 파일 하나로 관리
 *   - 앱 시작 시 Drive에서 읽기, 상태 변경 시 Drive에 덮어쓰기
 *   - Drive 파일이 없으면 자동 생성
 *
 * 인증 방식:
 *   - Google Identity Services (GIS) + gapi 사용
 *   - OAuth2 팝업으로 로그인
 */
...
import {
  ...
  GraduationCap, TrendingUp, Zap, LogIn, LogOut, Cloud, CloudOff, RefreshCw
} from "lucide-react";
...
const FILE_NAME = "career_data.json";
// drive.appdata: 앱 전용 숨김 폴더에 저장 — 사용자 Drive에 노출 안 됨
...
async function readCareerFile(fileId) {
...
async function createCareerFile(data) {
  const boundary = "career_boundary";
  const body = [
    `--${boundary}`,
...
async function updateCareerFile(fileId, data) {
...
const CERT_COLORS = ["#1e293b","#1e3a5f","#14532d","#7c2d12","#312e81","#4a1942","#064e3b","#1c1917"];
const EVENT_TYPES = {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="relative w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden"
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b" style={{ borderColor: "var(--border)" }}>
          ...
          <button onClick={onClose} className="p-1 rounded-lg hover:bg-white/10"><X size={16} style={{ color: "var(--text-muted)" }} /></button>
        </div>
...
function Input({ label, ...props }) {
  return (
    <div className="flex flex-col gap-1">
      ...
      <input {...props} className="w-full rounded-lg px-3 py-2 text-sm outline-none"
...
function Textarea({ label, ...props }) {
  return (
    <div className="flex flex-col gap-1">
      {label && <label className="text-xs font-medium" style={{ color: "var(--text-muted)" }}>{label}</label>}
      <textarea {...props} rows={3} className="w-full rounded-lg px-3 py-2 text-sm outline-none resize-none"
        style={{ background: "var(--input-bg)", border: "1px solid var(--border)", color: "var(--text-primary)" }} />
    </div>
  );
}
...
function Btn({ variant = "primary", size = "md", children, ...props }) {
  ...
  const v = variant === "primary" ? "text-white shadow-sm hover:opacity-90"
    : variant === "ghost" ? "hover:bg-white/10"
    : "border hover:bg-white/5";
  return (
    <button {...props} className={`${base} ${sz} ${v}`}
...
function CountdownBanner({ events }) {
  ...
  return (
    <div className="flex gap-3 flex-wrap mb-6">
      {upcoming.map(e => (
        <div key={e.id} className="flex items-center gap-3 rounded-xl px-4 py-3 flex-1 min-w-[200px]"
          style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
          <div className="rounded-lg p-2" style={{ background: EVENT_TYPES[e.type]?.color + "22" }}>
...
function Dashboard({ library, certs, events, setPage }) {
  ...
  const nextExam = events.filter(e => e.isDday && diffDays(e.date) >= 0)
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold mb-1" style={{ color: "var(--text-primary)", fontFamily: "var(--font-display)" }}>대시보드</h2>
...
function Library({ library, onChange }) {
  ...
  function addFile(sectionId) {
    ...
    onChange(library.map(s => s.id !== sectionId ? s : {
      ...s, files: [...s.files, { id: uid(), name: uploadName.trim() + "." + uploadType, date: today(), size: Math.floor(Math.random() * 4096 + 128) + "KB", type: uploadType }]
    }));
    setUploadTarget(null); setUploadName(""); setUploadType("pdf");
  }
  function deleteFile(sectionId, fileId) {
    onChange(library.map(s => s.id !== sectionId ? s : { ...s, files: s.files.filter(f => f.id !== fileId) }));
  }
    ...
    <div>
      ...
      {library.length === 0 && (
        <div className="rounded-xl p-12 text-center" style={{ border: "1.5px dashed var(--border)" }}>
          ...
          <p className="text-sm" style={{ color: "var(--text-muted)" }}>섹션이 없습니다.</p>
        </div>
      )}
        ...
        {library.map(section => (
          <div key={section.id} className="rounded-xl overflow-hidden"
            ...
            <div className="flex items-center gap-3 px-5 py-3.5"
              ...
              {editId === section.id ? (
                <input value={editName} onChange={e => setEditName(e.target.value)}
                  ...
                  style={{ color: "var(--text-primary)", borderColor: section.color }} autoFocus />
              ) : (
              ...
              {editId === section.id
                ? <button onClick={() => saveEdit(section.id)} className="p-1 hover:opacity-70"><Check size={13} style={{ color: section.color }} /></button>
                : <button onClick={() => { setEditId(section.id); setEditName(section.subject); }} className="p-1 hover:opacity-70"><Edit2 size={13} style={{ color: "var(--text-muted)" }} /></button>
              }
              <button onClick={() => setUploadTarget(section.id)}
                className="flex items-center gap-1 text-xs px-2.5 py-1 rounded-lg hover:opacity-80"
            ...
            {section.files.length === 0 ? (
              <div className="px-5 py-4 text-xs" style={{ color: "var(--text-muted)" }}>파일이 없습니다.</div>
            ) : (
              <div className="divide-y" style={{ borderColor: "var(--border)" + "66" }}>
                {section.files.map(f => {
                  const FIcon = getFileIcon(f.name);
                  return (
                    <div key={f.id} className="flex items-center gap-3 px-5 py-3 hover:bg-white/5">
                      ...
                      <span className="text-xs" style={{ color: "var(--text-muted)" }}>{formatDate(f.date)}</span>
      ...
      {uploadTarget && (
        <Modal title="파일 추가 (시뮬레이션)" onClose={() => setUploadTarget(null)}>
          <div className="flex flex-col gap-4">
            ...
            <Input label="파일명 (확장자 제외)" value={uploadName} onChange={e => setUploadName(e.target.value)} placeholder="예: 1주차 강의노트" />
            <Select label="파일 형식" value={uploadType} onChange={e => setUploadType(e.target.value)}>
              {["pdf","docx","pptx","xlsx","mp4","jpg","png","zip"].map(t => <option key={t} value={t}>{t.toUpperCase()}</option>)}
            </Select>
            <div className="flex gap-2 justify-end pt-2">
...
function Certificates({ certs, onChange }) {
  ...
  function deleteCert(id) { onChange(certs.filter(c => c.id !== id)); setDeleteId(null); }
    ...
    <div>
      ...
      {certs.length === 0 && (
        <div className="rounded-xl p-12 text-center" style={{ border: "1.5px dashed var(--border)" }}>
          ...
          <p className="text-sm" style={{ color: "var(--text-muted)" }}>자격증이 없습니다.</p>
        </div>
      )}
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {certs.map(cert => {
          ...
          return (
            <div key={cert.id} className="rounded-xl overflow-hidden relative group transition-all hover:scale-[1.02] hover:shadow-xl"
              ...
              <div className="p-5 pb-4">
                ...
                <h3 className="mt-4 text-base font-bold leading-tight" style={{ color: "rgba(255,255,255,0.95)" }}>{cert.name}</h3>
                {cert.issuer && <p className="text-xs mt-1" style={{ color: "rgba(255,255,255,0.6)" }}>{cert.issuer}</p>}
              </div>
              <div className="px-5 py-3 flex items-center justify-between" style={{ background: "rgba(0,0,0,0.2)" }}>
                <span className="text-xs" style={{ color: "rgba(255,255,255,0.7)" }}>취득: {formatDate(cert.date) || "-"}</span>
                {status && <span className="text-xs rounded-full px-2 py-0.5" style={{ background: status.color + "33", color: status.color }}>{status.text}</span>}
              </div>
              {cert.note && <div className="px-5 py-2 text-xs" style={{ color: "rgba(255,255,255,0.5)", background: "rgba(0,0,0,0.15)" }}>{cert.note}</div>}
            </div>
          );
        })}
      </div>
      {showAdd && (
        <Modal title="자격증 추가" onClose={() => setShowAdd(false)}>
          <div className="flex flex-col gap-3">
            <Input label="자격증명 *" value={form.name} onChange={f("name")} placeholder="예: TOEIC 900" />
            <Input label="발급 기관" value={form.issuer} onChange={f("issuer")} placeholder="예: ETS" />
      ...
      {deleteId && (
        <Modal title="삭제 확인" onClose={() => setDeleteId(null)}>
          <p className="text-sm mb-4" style={{ color: "var(--text-secondary)" }}>이 자격증을 삭제하시겠습니까?</p>
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          <div className="flex rounded-lg overflow-hidden" style={{ border: "1px solid var(--border)" }}>
            {["month","week"].map(v => (
              <button key={v} onClick={() => setView(v)} className="px-3 py-1.5 text-xs font-medium transition-all"
                style={{ background: view === v ? "var(--accent)" : "transparent", color: view === v ? "white" : "var(--text-muted)" }}>
                {v === "month" ? "월간" : "주간"}
              </button>
            ))}
          </div>
          <Btn onClick={() => { setForm(p => ({ ...p, date: today() })); setShowAdd(true); }}><Plus size={14} />일정 추가</Btn>
        </div>
      </div>
      <div className="flex items-center justify-between mb-4">
        <button onClick={() => view === "month" ? setCursor(p => new Date(p.getFullYear(), p.getMonth()-1, 1)) : setCursor(p => { const d = new Date(p); d.setDate(d.getDate()-7); return d; })}
          className="p-1.5 rounded-lg hover:bg-white/10"><ChevronLeft size={16} style={{ color: "var(--text-muted)" }} /></button>
        ...
        <button onClick={() => view === "month" ? setCursor(p => new Date(p.getFullYear(), p.getMonth()+1, 1)) : setCursor(p => { const d = new Date(p); d.setDate(d.getDate()+7); return d; })}
          className="p-1.5 rounded-lg hover:bg-white/10"><ChevronRight size={16} style={{ color: "var(--text-muted)" }} /></button>
      </div>
          ...
          <div className="grid grid-cols-7">
            {WEEKDAYS.map((d, i) => (
              <div key={d} className="py-2 text-center text-xs font-medium"
                style={{ color: i===0?"#ef4444":i===6?"#6366f1":"var(--text-muted)", background: "var(--surface)", borderBottom: "1px solid var(--border)" }}>{d}</div>
            ))}
            {monthGrid.map((day, i) => {
              ...
              return (
                <div key={i} onClick={() => { if(day){setForm(p=>({...p,date:ds}));setShowAdd(true);}}}
                  className="min-h-[80px] p-1.5 cursor-pointer hover:bg-white/5"
                  style={{ background: day?"transparent":"var(--surface)"+"44", borderRight:(i+1)%7!==0?"1px solid var(--border)":"none", borderBottom:i<monthGrid.length-7?"1px solid var(--border)":"none" }}>
                  {day && <>
                    <div className="flex justify-end">
                      <span className="text-xs w-6 h-6 flex items-center justify-center rounded-full font-medium"
                        style={{ background: isToday?"var(--accent)":"transparent", color: isToday?"white":isWeekend?(day.getDay()===0?"#ef4444":"#6366f1"):"var(--text-muted)" }}>
                        {day.getDate()}
                      </span>
                    </div>
                    <div className="flex flex-col gap-0.5 mt-0.5">
                      {dayEvents.slice(0,2).map(e => (
                        <div key={e.id} onClick={ev=>ev.stopPropagation()}
                          ...
                          <button onClick={ev=>{ev.stopPropagation();deleteEvent(e.id);}} className="opacity-0 group-hover:opacity-100 ml-1"><X size={9}/></button>
                        </div>
                      ))}
                      {dayEvents.length>2 && <div className="text-xs px-1" style={{color:"var(--text-muted)"}}>+{dayEvents.length-2}개</div>}
                    </div>
                  </>}
                </div>
              );
        ...
        <div className="rounded-xl overflow-hidden" style={{ border: "1px solid var(--border)" }}>
          <div className="grid grid-cols-7">
            {weekDates.map((day, i) => {
              ...
              const dayEvents = eventsOnDate(day);
              return (
                <div key={i} className="cursor-pointer hover:bg-white/5"
                  onClick={() => {setForm(p=>({...p,date:ds}));setShowAdd(true);}}
                  style={{ borderRight: i<6?"1px solid var(--border)":"none" }}>
                  <div className="py-2 text-center" style={{ borderBottom:"1px solid var(--border)", background:"var(--surface)" }}>
                    <div className="text-xs mb-1" style={{ color: i===0?"#ef4444":i===6?"#6366f1":"var(--text-muted)" }}>{WEEKDAYS[i]}</div>
                    <div className="text-base font-bold mx-auto w-8 h-8 flex items-center justify-center rounded-full"
                      style={{ background: isToday?"var(--accent)":"transparent", color: isToday?"white":"var(--text-primary)" }}>
                      {day.getDate()}
                    </div>
                  </div>
                  <div className="p-1 min-h-[120px] flex flex-col gap-1">
                    {dayEvents.map(e => (
                      <div key={e.id} onClick={ev=>ev.stopPropagation()}
                        ...
                        style={{ background: EVENT_TYPES[e.type]?.color+"22", color: EVENT_TYPES[e.type]?.color }}>
                        <span className="truncate">{e.isDday&&"★ "}{e.title}</span>
                        <button onClick={ev=>{ev.stopPropagation();deleteEvent(e.id);}} className="opacity-0 group-hover:opacity-100"><X size={9}/></button>
                      </div>
                    ))}
                  </div>
                </div>
                  ...
                  <div key={e.id} className="flex items-center gap-3 px-4 py-3 rounded-xl"
                    ...
                    {e.isDday && (
                      <div className="text-xs font-bold tabular-nums flex-shrink-0 px-2 py-0.5 rounded-full"
                        style={{ background: diff<0?"#94a3b822":diff===0?"#ef444422":"var(--accent)"+"22", color: diff<0?"#94a3b8":diff===0?"#ef4444":"var(--accent)" }}>
                        {dDayLabel(diff)}
                      </div>
                    )}
      ...
      {showAdd && (
        <Modal title="일정 추가" onClose={() => setShowAdd(false)}>
          <div className="flex flex-col gap-3">
            <Input label="제목 *" value={form.title} onChange={f("title")} placeholder="예: TOEIC 시험" />
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={form.isDday} onChange={e => setForm(p => ({ ...p, isDday: e.target.checked }))} className="w-4 h-4 rounded" />
...
// 10. SIDEBAR
// ─────────────────────────────────────────────
function Sidebar({ page, setPage, certs, events, syncStatus, onSignIn, onSignOut, isSignedIn }) {
  ...
  return (
    <nav className="flex flex-col h-full" style={{ width: "220px", minWidth: "220px" }}>
      <div className="px-5 py-5 flex items-center gap-2.5 mb-2">
        ...
        <div>
          <div className="text-sm font-bold" style={{ color: "var(--text-primary)", fontFamily: "var(--font-display)", letterSpacing: "-0.01em" }}>CareerKit</div>
      ...
      <div className="flex-1 flex flex-col gap-0.5 px-3">
        {navItems.map(item => (
          <button key={item.id} onClick={() => setPage(item.id)}
            ...
            style={{ background: page===item.id?"var(--accent)"+"18":"transparent", color: page===item.id?"var(--accent)":"var(--text-secondary)", border: page===item.id?"1px solid "+"var(--accent)"+"30":"1px solid transparent" }}>
      ...
      <div className="px-4 py-4 mt-auto flex flex-col gap-2">
        <div className="flex items-center gap-2 px-1">
          {syncStatus === "synced"  && <><Cloud size={12} style={{color:"#10b981"}}/><span className="text-xs" style={{color:"#10b981"}}>Drive 동기화됨</span></>}
          {syncStatus === "syncing" && <><RefreshCw size={12} style={{color:"var(--accent)"}} className="animate-spin"/><span className="text-xs" style={{color:"var(--accent)"}}>저장 중...</span></>}
          {syncStatus === "offline" && <><CloudOff size={12} style={{color:"var(--text-muted)"}}/><span className="text-xs" style={{color:"var(--text-muted)"}}>로컬 저장</span></>}
          {syncStatus === "error"   && <><CloudOff size={12} style={{color:"#ef4444"}}/><span className="text-xs" style={{color:"#ef4444"}}>동기화 오류</span></>}
        </div>
        {isSignedIn
          ? <button onClick={onSignOut} className="flex items-center gap-2 px-3 py-2 rounded-lg w-full hover:bg-white/10 transition-colors">
              <LogOut size={13} style={{color:"var(--text-muted)"}}/>
              <span className="text-xs" style={{color:"var(--text-muted)"}}>로그아웃</span>
            </button>
          : <button onClick={onSignIn} className="flex items-center gap-2 px-3 py-2 rounded-lg w-full transition-colors"
              style={{background:"var(--accent)"+"18", border:"1px solid var(--accent)"+"30"}}>
              <LogIn size={13} style={{color:"var(--accent)"}}/>
              <span className="text-xs font-medium" style={{color:"var(--accent)"}}>Google 로그인</span>
            </button>
        }
      </div>
    </nav>
...
// 11. ROOT APP
// ─────────────────────────────────────────────
const EMPTY = { library: [], certs: [], events: [] };
...
export default function App() {
  ...
  // ── Load scripts & init gapi ──
  useEffect(() => {
    Promise.all([loadGapiScript(), loadGisScript()]).then(async () => {
      await initGapi();
      setGapiReady(true);
  ...
  async function loadFromDrive() {
    ...
    try {
      const file = await findCareerFile();
      if (file) {
        fileIdRef.current = file.id;
        const loaded = await readCareerFile(file.id);
        setData({ library: loaded.library||[], certs: loaded.certs||[], events: loaded.events||[] });
      } else {
        // 파일 없으면 빈 데이터로 새로 만들기
        const newId = await createCareerFile(EMPTY);
  ...
  function scheduleSave(newData) {
    ...
    saveTimerRef.current = setTimeout(async () => {
      try {
        await updateCareerFile(fileIdRef.current, newData);
        setSyncStatus("synced");
      } catch (e) {
        console.error(e);
        setSyncStatus("error");
  ...
  // ── State updaters ──
  function updateLibrary(library) { const d = {...data, library}; setData(d); scheduleSave(d); }
  function updateCerts(certs)     { const d = {...data, certs};   setData(d); scheduleSave(d); }
  function updateEvents(events)   { const d = {...data, events};  setData(d); scheduleSave(d); }
  ...
  const theme = {
    "--bg": "#0b0f17", "--surface": "#141925", "--surface-2": "#1c2333",
    "--border": "#ffffff12", "--accent": "#6366f1",
    "--text-primary": "#f1f5f9", "--text-secondary": "#94a3b8", "--text-muted": "#4b5563",
    "--input-bg": "#1c2333", "--font-display": "'DM Serif Display', Georgia, serif",
  };

  const pageComponents = {
    dashboard: <Dashboard library={data.library} certs={data.certs} events={data.events} setPage={setPage} />,
    library:   <Library   library={data.library} onChange={updateLibrary} />,
        ...
        <div className="flex h-screen overflow-hidden">
          <div className="hidden md:flex flex-col flex-shrink-0 py-2"
            ...
            <Sidebar page={page} setPage={setPage} certs={data.certs} events={data.events}
              syncStatus={syncStatus} onSignIn={handleSignIn} onSignOut={handleSignOut} isSignedIn={isSignedIn} />
          </div>
          {sidebarOpen && (
              ...
              {!isSignedIn && (
                <div className="mb-4 rounded-xl px-4 py-3 flex items-center gap-3"
                  ...
                  <span className="text-sm flex-1" style={{ color: "var(--accent)" }}>
                    Google 로그인하면 데이터가 Drive에 자동 저장됩니다.
                  </span>
                  <button onClick={handleSignIn} className="text-xs font-semibold px-3 py-1.5 rounded-lg"
